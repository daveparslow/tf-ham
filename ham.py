...
class HAMTree(object):
    ...
    def construct(self, total_leaves):
        ...
        # [[A B] [C D]]
        ###
        queue = [HAMNode(tree=self, left=None, right=None)
...
class HAMNode(object):
    ...
    def retrieve_and_update(self, control, attention=1.0):
        ...
        if self.left and self.right:
            ...
            value = self.right.retrieve_and_update(
                control, attention=attention * move_right)
            value += self.left.retrieve_and_update(
                control, attention=attention * (1 - move_right))
        else:
        ...
        if self.left and self.right:
            ...
            self.h = attention * \
                self.tree.write(self.h, control) + (1 - attention) * self.h
        return value
...
if __name__ == '__main__':
    ...
    assert len(
        tree.nodes) == 3, 'Depth 2 tree should have 3 nodes (two leaves, one root)'
