...
tf.app.flags.DEFINE_integer(
    'batches_per_epoch', 1000, 'Number of batches per epoch')
...
tf.app.flags.DEFINE_integer(
    'embed_size', 10, 'Embedding size for input to sort - half key, half value')
tf.app.flags.DEFINE_integer('tree_size', 20, 'Tree size')
tf.app.flags.DEFINE_integer('controller_size', 20, 'Controller size')
tf.app.flags.DEFINE_string('weights_path', './ham.weights',
                           'Where to load and save the weights for the model')
tf.app.flags.DEFINE_boolean('test', False, 'Whether to only run testing')

config = tf.app.flags.FLAGS

inputs = tf.placeholder(tf.float32, shape=[
                        config.batch_size, config.n, config.embed_size], name='Input')
control = tf.placeholder(
    tf.float32, shape=[config.batch_size, config.controller_size], name='Control')
target = tf.placeholder(tf.float32, shape=[
                        config.batch_size, config.n, config.embed_size], name='Target')

ham_ops = HAMOperations(
    config.embed_size, config.tree_size, config.controller_size)
...
calculate_predicted = tf.concat(
    axis=1, values=[tree.get_output(control) for _ in range(config.n)])
targets = tf.reshape(target, [config.batch_size, config.n * config.embed_size])


def penalty(x, y): return tf.pow(x - y, 2)


#penalty = lambda x, y: tf.abs(x - y)
calculate_loss = tf.reduce_sum(
    penalty(calculate_predicted, targets)) / config.n / config.batch_size
...
with tf.Session() as session:
    ...
    summary_writer = tf.summary.FileWriter(
        '/tmp/ham-sort/', session.graph_def, flush_secs=5)
    ...
    for epoch in range(config.max_epochs):
        ...
        for i in range(total_batches):
            ...
            for b in range(config.batch_size):
                x, y = create_example(
                    n=config.n, bit_length=int(config.embed_size / 2))
                X.append(x)
                Y.append(y)
            control_signal = np.zeros(
                [config.batch_size, config.controller_size], dtype=np.float32)
            feed = {inputs: X, target: Y, control: control_signal}
            _, loss, predicted = session.run(
                [tf.no_op() if config.test else train_step, calculate_loss, calculate_predicted], feed_dict=feed)
            ###
            for b in range(config.batch_size):
